<template>
  <div class="catagorey-list">
    ...
    >
      ...
      >
        <span slot="Particulars" slot-scope="text">{{ text }}</span>
        <span slot="Quotes" slot-scope="text">{{ text }}</span>
        <a slot="Sales Orders" slot-scope="text">{{ text }}</a>

        <a slot="inquiries" slot-scope="text">{{ text }}</a>
        ...
        <div slot="action" slot-scope="text, record">
          <a-button-group>
            ...
            <a-button
              type="default"
              @click="$router.push(`/lc-infos/${record.id}/edit`)"
              icon="edit"
            />
            <a-button
              type="default"
              @click="$router.push(`/lc-infos/${record.id}/details`)"
              icon="info"
            />
          </a-button-group>
        </div>
...
const columns = [
  {
    ...
    sorter: true,
    width: "20%",
    scopedSlots: { customRender: "Particulars" },
  },

  {
    ...
    dataIndex: "Quotes",
    key: "Quotes",
    scopedSlots: { customRender: "Quotes" },
  },
  {
    title: "Sales Orders",
    dataIndex: "Sales",
    key: "Sales Orders",
    sorter: true,
    scopedSlots: { customRender: "Sales Orders" },
  },
  {
    title: "Inquiries",
    dataIndex: "inquiries",
    key: "inquiries",
    sorter: true,
    scopedSlots: { customRender: "Inquiries" },
  },
  {
    title: "Active Status",
    dataIndex: "",
